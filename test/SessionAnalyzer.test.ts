...
describe("SessionAnalyzer", () => {
  ...
  test("analyze() returns null when no transcript", () => {
    const input = { session_id: "test" };
    const result = SessionAnalyzer.analyze(input);
    expect(result).toBeNull();
  });

  test("analyze() with budget.percent returns null", () => {
    const input = {
      session_id: "test",
      budget: { percent: 75 },
    };
  ...
  test("analyze() with cost.percent returns null", () => {
    const input = {
      session_id: "test",
      cost: { percent: 50 },
    };
  ...
  test("analyze() with transcript path", () => {
    const lines = [
      JSON.stringify({
        type: "assistant",
        message: { usage: { input_tokens: 1000 } },
    ...
    const input = { session_id: "test" };
    const result = SessionAnalyzer.analyze(input, transcriptPath);

    expect(result).not.toBeNull();
    expect(result?.usedTokens).toBe(1000);
  });

  test("analyze() with percent in input returns null", () => {
    const input = {
      session_id: "test",
      someField: { percent: 90 },
    };
    const result = SessionAnalyzer.analyze(input);
    expect(result).toBeNull();
  });
